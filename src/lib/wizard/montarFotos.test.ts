...
describe("montarFotosParaCava", () => {
  it("Cava Furada gera 1 foto", () => {
    ...
    expect(fotos).toEqual([{ cava: 1, tipoCava: "Cava Furada", fotoNum: 1, label: "Durante a exploracao" }]);
  });

  it("Cava Normal gera 5 fotos, na ordem certa", () => {
    const fotos = montarFotosParaCava("Cava Normal", 1);
    expect(fotos).toHaveLength(5);
    expect(fotos[0]).toEqual({ cava: 1, tipoCava: "Cava Normal", fotoNum: 1, label: "Antes de explorar o solo" });
    expect(fotos[4]).toEqual({
      cava: 1,
      tipoCava: "Cava Normal",
      fotoNum: 5,
      label: "Cava pronta com trena medindo a profundidade",
    });
  });

  it("numera a cava certa quando é a segunda ou terceira do turno", () => {
    expect(montarFotosParaCava("Cava Furada", 3)).toEqual([
      { cava: 3, tipoCava: "Cava Furada", fotoNum: 1, label: "Durante a exploracao" },
    ]);
  });
